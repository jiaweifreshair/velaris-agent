...
class BundlePlanner:
    ...
    def __init__(
        ...
        *,
        ...
        basis_builder: DecisionBasisBuilder | None = None,
    ) -> None:
        """初始化编排器，允许注入子模块实现。"""

        self._feasibility_checker = feasibility_checker or FeasibilityChecker()
        self._basis_builder = basis_builder or DecisionBasisBuilder()

    def evaluate(self, request: BundleDecisionRequest) -> BundleDecisionResponse:
        """执行共享决策请求。"""

        if request.bundle_candidates or request.decision_type == "bundle_rank":
            return self._evaluate_bundle_rank(request)
        return self._evaluate_domain_rank(request)

    def _evaluate_domain_rank(self, request: BundleDecisionRequest) -> BundleDecisionResponse:
        """执行同类候选排序。"""
        ...
        feasible_candidates, rejected_reasons = self._feasibility_checker.filter_candidates(
        ...
        if not feasible_candidates:
            return BundleDecisionResponse(
                ...
                hard_constraint_report={
                    "passed": False,
                    "checks": self._feasibility_checker.summarize_hard_constraints(
                        candidate_set.hard_constraints,
                    ),
        ...
        ranked_candidates = [
            RankedCandidate(
                ...
                hard_constraint_report={
                    "passed": True,
                    "checks": self._feasibility_checker.candidate_checks(
                        candidate_map[item["id"]],
                        candidate_set.hard_constraints,
                    ),
                },
                reason=self._basis_builder.build_reason_from_scores(
                    item["scores"],
                    kind="candidate",
                ),
            )
        ...
        basis = self._basis_builder.build_candidate_basis(
            ranked_candidates=ranked_candidates,
            rejected_reasons=rejected_reasons,
            hard_constraints=candidate_set.hard_constraints,
        )
        hard_constraint_report = self._feasibility_checker.build_candidate_report(
            feasible_candidates=ranked_candidates,
            rejected_reasons=rejected_reasons,
            hard_constraints=candidate_set.hard_constraints,
        )
        return BundleDecisionResponse(
            ...
            why_selected=basis["why_selected"],
            why_not_others=basis["why_not_others"],
            tradeoffs=basis["tradeoffs"],
            uncertainty_flags=self._feasibility_checker.build_uncertainty_flags(
                hard_constraints=candidate_set.hard_constraints,
                feasible_count=len(ranked_candidates),
            ),
            explanation_text=basis["explanation_text"],
            decision_trace_id=_build_trace_id("hotel-biztravel"),
        )

    def _evaluate_bundle_rank(self, request: BundleDecisionRequest) -> BundleDecisionResponse:
        """执行跨类 bundle 联合排序。"""

        feasible_bundles, rejected_reasons = self._feasibility_checker.filter_bundles(
            request.bundle_candidates,
        ...
        basis = self._basis_builder.build_bundle_basis(
            ranked_bundles=ranked_bundles,
            rejected_reasons=rejected_reasons,
            hard_constraints=request.hard_constraints,
        )
        hard_constraint_report = self._feasibility_checker.build_bundle_report(
            feasible_bundles=feasible_bundles,
...
_DEFAULT_BUNDLE_PLANNER: BundlePlanner | None = None


def evaluate_bundle_decision(request: BundleDecisionRequest) -> BundleDecisionResponse:
    """执行共享决策请求。"""

    global _DEFAULT_BUNDLE_PLANNER
    if _DEFAULT_BUNDLE_PLANNER is None:
        _DEFAULT_BUNDLE_PLANNER = BundlePlanner()
    return _DEFAULT_BUNDLE_PLANNER.evaluate(request)
