...
Velaris 不是一个评分函数，也不是又一个 Agent 框架。它是一个**目标导向的本体决策智能体运行时（Ontology-Guided Goal-Oriented Decision Agent Runtime）**，为 AI 智能运营商提供基座引擎。

```
通用 Agent 框架:  用户提问 -> 调 API -> 返回结果 -> 忘掉一切
自改进 Agent:     用户提问 -> 执行任务 -> 沉淀技能 -> 下次执行更快
Velaris:          用户目标 -> Ontology 建模(谁/什么关系/什么目标) ->
                  自主抓取数据 -> 参考历史决策 -> 个性化评分 ->
                  推荐+解释 -> 快照决策环境 -> 学习偏好 -> 自进化
```

**核心理念**: 决策过程本身是资产。同样的 LLM，谁的决策上下文更完整、治理更显式、学习闭环更紧、决策主体建模越本质（Ontology），谁的决策就更准。

### 基座引擎定位

Velaris 解决的不是"让 Agent 能干活"或"让 Agent 越干越好"，而是**让 Agent 的决策过程可控、可审计、可回放、可学习——且始终以目标为导向（Goal-Oriented），从本体论层面（Ontology）建模决策主体**。

决策引擎覆盖三类**本体决策主体（Ontological Decision Subjects）**及其关系：
...
|------|----------|--------|---------|
| **定位** | 通用自主 Agent 框架 | 自我改进的 AI 队友 | **本体决策智能体运行时 / 基座引擎** |
| **核心创新** | Gateway + ReAct 循环 + 社区技能生态 | 闭环学习（技能自创建 + 自改进） | **目标导向 + Ontology 决策主体建模** + 治理闭环（路由 + 授权 + 账本 + Outcome + 快照） |
| **学习对象** | 无（技能是静态的） | 技能级（完成任务后自动写技能） | **决策级（Ontology 主体 + 用户偏好 + 组织策略 + 偏差纠正）** |
| **治理模型** | 基本无（prompt 级约束） | 轻量（权限 + 审批） | 显式策略路由 + 能力令牌 + 任务账本 + 停止条件 |
| **决策主体** | 单一（用户） | 单一（用户） | **本体三元组（用户 + 组织 + 关系对齐）+ Multi-Agent 就绪** |
...
一句话总结：OpenClaw 让 Agent 能干活，Hermes 让 Agent 越干越好，**Velaris 让 Agent 从本体论层面理解决策主体、以目标为导向做出可审计可进化的决策**。
...
### 五层 + 学习闭环设计 — 新一代目标导向本体决策智能体

下面这组分层说明从治理与编排视角展开，和上面的图集是互补关系。

```
╔══ Ontology 决策主体 ═══════════════════════════════════════════╗
║  ⬡ 用户决策 · 组织决策 · 关系对齐 Alignment                    ║
║  🎯 目标导向: Intent → Plan → Execute → Learn (闭环)          ║
║  🔗 Multi-Agent 接入: Claude Code · Codex · OpenClaw · Hermes · Gemini
╚═══════════════════════════════════════════════════════════════╝
                              ↓ 驱动
...
**Ontology 决策主体（第 0 层 — 哲学与驱动层）**— Velaris 的核心差异化：**一切围绕目标导向决策而构建的新一代本体论智能体**。参考哲学上的 Ontology（本体论）概念——"存在与存在的本质"——Velaris 将决策主体本身作为第一公民：

- **三元决策主体模型**：用户决策（个人偏好+行为模式）+ 组织决策（策略约束+硬约束）+ 关系对齐（AlignmentReport 冲突/协同/协商空间）。三者不是简单的加权，而是通过 Ontology 层统一建模为"谁在什么目标下、基于什么本质关系、做出什么决策"
- **目标导向闭环**：Intent（意图理解）→ Plan（能力规划）→ Execute（执行+评分）→ Learn（偏好进化+技能自进化），形成以目标为圆心的完整闭环，而非传统的"提问-回答"线性模式
- **Multi-Agent 编排就绪**：原生支持 Claude Code / Codex (OpenAI) / OpenClaw / Hermes / Gemini CLI 等多 Agent 平台接入，每个平台通过 Adapter 适配为统一的 StructuredDataSource；路由策略 (`routing-policy.yaml`) 支持本地闭环 / 委托 / 混合模式四种运行策略，可灵活编排任意 Multi-Agent 协作场景

**Layer 0: Agent Loop (Ontology-Driven Runtime)** — 基于 OpenHarness engine，由 **Ontology 决策主体层驱动**。支持 **Multi-Agent 接入**（Claude Code / Codex-OpenAI / OpenClaw / Hermes / Gemini CLI），流式 LLM 推理 + 多轮工具调用编排。通过统一 MCP 协议适配不同平台的工具生态，不锁定单一供应商。每个请求从 Ontology 层继承目标意图和决策主体上下文（用户/组织/关系三元组），确保推理过程始终围绕目标展开。
...
**Multi-Agent 接入层** — **Ontology 层原生支持 Multi-Agent 编排**，统一接入 5 大平台：Claude Code、Codex (OpenAI)、OpenClaw、Hermes、Gemini CLI。各平台通过 Adapter 层适配为统一的 StructuredDataSource，路由策略 (`routing-policy.yaml`) 支持本地闭环 / 委托 Claude Code / 委托 RobotClaw / 混合模式四种运行策略。**可接入任意 Multi-Agent 协作场景**——Velaris 不绑定单一 Agent 平台，而是作为决策智能体运行时在多个 Agent 之间编排目标驱动的决策流程。
...
```
用户目标 → Ontology 建模(主体+关系) → Agent 自主抓取数据 → 决策环境快照
  → 偏好学习 + 语义召回 → 权重更新 → 推荐更准 (Goal-Oriented)
                                              ↓
                                    SkillEvolutionLoop → 技能自进化
                                    CostOptimizer → Token 经济学优化
```

每次决策的完整环境（意图 + 本体主体关系 + 抓取到的数据 + 评分 + 路由 trace + 用户选择 + 满意度）都被快照记录。PreferenceLearner 从实际选择中学习个性化权重，SemanticRecallEngine 语义召回相似历史，SkillEvolutionLoop 闭环进化技能，CostOptimizer 优化 Token 开销。**Ontology 层确保所有学习和优化都围绕目标展开**——用得越多，推荐越准、成本越低、目标对齐度越高。
